...
> "Based on what you know from the other projects, create a C++ implementation of the JokeAPI in the cpp directory. The API must conform to what's described in joke.api. Create unit tests that test the functionality at the domain and persistence layers. Make sure the unit tests pass, then make sure the scripts in the tests directory pass. When the tests all pass and the functionality is equivalent to the other projects, create a Docker image that uses the host's port 8000 to accept traffic. Then run the Docker image and test it using the curl scripts again."
